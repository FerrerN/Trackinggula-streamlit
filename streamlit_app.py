...
class SugarTracker:
    ...
    """
    # --- Perbaikan Bug: __init__ harus menggunakan dua garis bawah ---
    def __init__(self):
...
if menu == "Laporan Harian":
    ...
    if not st.session_state.tracker.user_profile:
        ...
        with col2:
            st.markdown(f'<div class="metric-card"><h3>❤️ Batas AHA</h3><p>{aha_val} g</p></div>', unsafe_allow_html=True)
        with col3:
    ...
    if not st.session_state.tracker.user_profile:
        st.warning("⚠️ Silakan atur profilmu terlebih dahulu.")
    else:
    ...
    if not st.session_state.tracker.user_profile:
        st.warning("⚠️ Atur profil untuk melihat grafik dengan batas rekomendasi.")
